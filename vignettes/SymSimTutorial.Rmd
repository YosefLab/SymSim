...
**nPCR**  number of PCR cycles in the pre-amplification step;  
...
observed_counts <- True2ObservedCounts(true_counts=true_counts_res[[1]], meta_cell=true_counts_res[[3]], protocol="nonUMI", alpha_mean=0.1, alpha_sd=0.05, lenslope=0.01, nbins=20, gene_len=gene_len, amp_bias_limit=c(-0.3, 0.3), rate_2PCR=0.7, nPCR=16, depth_mean=1e5, depth_sd=3e3)
```
...
```{r,cache=T}
true_counts_res <- SimulateTrueCounts(ncells_total=500, ngenes=ngenes, evf_center=1, nevf=20, evf_type="continuous", n_de_evf=12, impulse = F, vary="s", Sigma=0.4, phyla=Phyla5(), gene_effects_sd=1, gene_effect_prob=0.3, bimod=0, randseed=0, param_realdata = "zeisel.imputed", SE=F)
...
load("SymSim/data/cortex_data_pop3.RData")
best_matches_UMI <- BestMatchParams('UMI',cortex_counts,'best_params.umi.qqplot.pdf', n_optimal=8)
best_matches_UMI
```
Given the Th17 dataset:
```{r,cache=T, fig.width=6.6, fig.height=2.4}
load("SymSim/data/Th17_data.RData")
counts <- ss2_130cells_counts[which(rowSums(ss2_130cells_counts > 1) > 10),]
best_matches_nonUMI <- BestMatchParams('nonUMI',counts,'best_params.nonUMI.qqplot.pdf', n_optimal=8)
best_matches_nonUMI
```