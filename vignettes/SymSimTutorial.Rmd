...
true_counts_res <- SimulateTrueCounts(ncells_total=400, min_popsize=40, i_minpop=1, ngenes=ngenes, 
                                      evf_center=1, nevf=10, evf_type="discrete", percent_DEevf=0.5, Sigma=0.2, 
                                      phyla=Phyla5(), gene_effects_sd=1, gene_effect_prob=0.3, bimod=0.3,
                                      randseed=0, param_realdata = "zeisel.imputed", SE=F)

pcaRes <-  prcomp(true_counts_res[[3]][, 3:12], scale. = T)
plotPCAbasic(pcaRes, col_vec = true_counts_res[[3]][,2], figuretitle = "evfs")
legend("bottomright", legend = 1:5, col = 1:5, pch=20)

pca_counts <- prcomp(t(true_counts_res[[1]]), scale. = T)
tsne_after_pca <- PlotTsne(meta = true_counts_res[[3]], data= t(pca_counts$x[, 1:10]), label = "pop", saving = F)
tsne_after_pca[[2]]
...
pca_observed_counts <- prcomp(t(observed_counts[[1]][which(rowSums(observed_counts[[1]])>0),]), scale. = T)
tsne_after_pca_ss2 <- PlotTsne(meta=observed_counts[[2]], data= t(pca_observed_counts$x[, 1:10]), label = "pop", saving = F)
tsne_after_pca_ss2[[2]]
```
