...
SimulateCounts <- function(ncells,ngenes,nevf,nbatches,evfs,gene_effects,
	...
	if(length(batch[1,])!=nbatches){print('ERROR: number of columns if batch effect is equal to the number of bathces');stop()}
...
SimulateDataset <- function(ncells,ngenes,nbatch,nevf,
	...
	sim <- SimulateCounts(ncells,ngenes,nevf,nbatches,evfs=evf[[1]],
